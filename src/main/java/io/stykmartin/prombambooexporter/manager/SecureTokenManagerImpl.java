...
public class SecureTokenManagerImpl implements SecureTokenManager, InitializingBean {
	...
	public boolean isConfigured() {
		PluginSettings settings = getPluginSettings();
		return readString(settings, SALT_KEY) != null && readString(settings, HASH_KEY) != null;
	}
	...
	public boolean matches(String candidate) {
		...
		byte[] actual = hash(Base64.getDecoder().decode(saltB64), candidate.getBytes(StandardCharsets.UTF_8));
